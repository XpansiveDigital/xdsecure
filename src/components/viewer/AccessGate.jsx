...
// AccessGate: full-screen password gate for Private and Internal layers.
// TODO: Replace local password check with POST /api/guides/:id/authenticate

const GATE_CONFIG = {
  private: {
    badge:       'Private Access',
    badgeCls:    'bg-amber-500/15 text-amber-300/90 border-amber-500/25',
    title:       'Private Access',
    body:        'This guide is shared selectively with qualified clients and approved partners.',
    buttonLabel: 'Access Private View',
  },
  internal: {
    badge:       'Internal Use Only',
    badgeCls:    'bg-red-500/20 text-red-300/90 border-red-500/30',
    title:       'Internal Access Required',
    body:        'This section contains operational documents restricted to authorised team members.',
    buttonLabel: 'Access Internal View',
  },
}

export default function AccessGate({ layer, password, hint, onUnlock }) {
  const [input,    setInput]    = useState('')
  const [error,    setError]    = useState(false)
  const [showCode, setShowCode] = useState(false)

  const config = GATE_CONFIG[layer] || GATE_CONFIG.private
  ...
  return (
    <div className="flex-1 flex items-center justify-center bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 p-8 relative overflow-hidden">
      ...
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
        <div className="w-[32rem] h-[32rem] rounded-full bg-white/[0.03] blur-3xl" />
      </div>
      ...
      <div className="relative w-full max-w-sm">
        ...
        <div className="flex justify-center mb-7">
          <div className="relative">
            ...
            <div className="relative w-20 h-20 rounded-3xl bg-white/[0.06] border border-white/10 flex items-center justify-center backdrop-blur-sm">
              <svg className="w-10 h-10 text-white/60" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.25}>
        ...
        <div className="flex justify-center mb-4">
          <span className={`inline-flex items-center gap-1.5 text-[11px] font-semibold px-3 py-1 rounded-full border tracking-wide ${config.badgeCls}`}>
            {config.badge}
          </span>
        </div>
        ...
        <p className="text-center text-xs text-white/30 mt-6 leading-relaxed">
          {hint || "Don't have an access code? Contact your venue representative."}
        </p>
